...
IUSE="bindist ${HPN_PATCH:++}hpn kerberos kernel_linux ldap ldns libedit audit pam +pie sctp selinux skey static X X509"
...
src_prepare() {
	...
	if use selinux; then
		epatch ${FILESDIR}/${PN}-role-mls.patch
		epatch ${FILESDIR}/${PN}-privsep-selinux.patch
		use ldap || epatch ${FILESDIR}/${PN}-keycat-noldap.patch
		use ldap && epatch ${FILESDIR}/${PN}-keycat.patch
		epatch ${FILESDIR}/${PN}-selinux-contexts.patch
	fi
#	epatch ${FILESDIR}/${PN}-entropy.patch
	epatch ${FILESDIR}/${PN}-ctr-evp-fast.patch
	epatch ${FILESDIR}/${PN}-utf8-banner.patch
#	if use audit; then
#		epatch ${FILESDIR}/${PN}-audit.patch
#	fi
